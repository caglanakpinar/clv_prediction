...
def get_filters(results, customer_indicator, amount_indicator):
    data_pv = (
        results.groupby(customer_indicator).agg({amount_indicator: "sum"}).reset_index()
    )
    top_100_customers = list(
        data_pv.sort_values(amount_indicator, ascending=False)[customer_indicator]
    )[0:100]
    worst_100_customers = list(
        data_pv.sort_values(amount_indicator, ascending=True)[customer_indicator]
    )[0:100]
    ...
    values = ["all", "all", "sum"]
    filters = list(
        zip(filter_ids, filter_datas, filter_sizes, multiple_selection, values)
    )
...
def decide_time_period(date, time_period):
    ...
    if time_period == "week":  # only shows mondays for weekly comparison
        ...
        return datetime.datetime.strptime(
            str(date)[0:10], "%Y-%m-%d"
        ) - datetime.timedelta(days=week_day - 1)
    if time_period == "2*week":  # only shows mondays for weekly comparison
        ...
        return datetime.datetime.strptime(
            str(date)[0:10], "%Y-%m-%d"
        ) - datetime.timedelta(days=mondays_cal_day_count)
...
def get_new_comer_list(row, customer_list, time_p_ind, customer_indicator):
    prev_customers = list(
        customer_list[customer_list[time_p_ind] < row[time_p_ind]][customer_indicator]
    )
...
def get_churn_list(
    row, customer_list, time_p_ind, customer_indicator, max_time_indicator
):
    prediction_customers = list(
        customer_list[customer_list[time_p_ind] == max_time_indicator][
            customer_indicator
        ]
    )
    if len(row[customer_indicator]) != 0 and len(prediction_customers) != 0:
        ...
        return pd.Series(
            [
                diffs,
                len(diffs) / len(set(prediction_customers + row[customer_indicator])),
            ]
        )
    else:
        return pd.Series([[], 0])
...
def get_churn_and_new_comer_columns(
    data, time_p_ind, amount_indicator, customer_indicator
):
    data_pv = (
        data.groupby(time_p_ind)
        .agg(
            {
                amount_indicator + "_sum": "sum",
                amount_indicator + "_mean": "mean",
                customer_indicator: lambda x: list(np.unique(x)),
            }
        )
        .reset_index()
    )
    ...
    data_pv[["new_comer_list", "new_comer_ratio"]] = data_pv.apply(
        lambda row: get_new_comer_list(
            row, customer_list, time_p_ind, customer_indicator
        ),
        axis=1,
    )
    data_pv[["churn_list", "churn_ratio"]] = data_pv.apply(
        lambda row: get_churn_list(
            row, customer_list, time_p_ind, customer_indicator, predicted_time_period
        ),
        axis=1,
    )
    data = pd.merge(
        data,
        data_pv[[time_p_ind, "new_comer_list", "new_comer_ratio"]],
        on=time_p_ind,
        how="left",
    )
    data = pd.merge(
        data,
        data_pv[[time_p_ind, "churn_list", "churn_ratio"]],
        on=time_p_ind,
        how="left",
    )
...
def get_new_comer_churn_data(data, customer_indicator, amount_indicator, time_p_ind):
    data["is_new_comer"] = data.apply(
        lambda row: True if row[customer_indicator] in row["new_comer_list"] else False,
        axis=1,
    )
    data["is_churn"] = data.apply(
        lambda row: True if row[customer_indicator] in row["churn_list"] else False,
        axis=1,
    )
    new_comer_data = (
        data.query("is_new_comer == True")
        .groupby(time_p_ind)
        .agg(
            {
                amount_indicator + "_sum": "sum",
                amount_indicator + "_mean": "mean",
                "new_comer_ratio": "first",
            }
        )
        .reset_index()
    )
    churn_data = (
        data.query("is_churn == True")
        .groupby(time_p_ind)
        .agg(
            {
                amount_indicator + "_sum": "sum",
...
def pivoting_data_per_time_period(
    data,
    results,
    time_indicator,
    time_period,
    ...
    data[amount_indicator + "_sum"], data[amount_indicator + "_mean"] = (
        data[amount_indicator],
        data[amount_indicator],
    )
    data = data[data[amount_indicator] == data[amount_indicator]]
    data_pv = (
        data.groupby([time_p_ind, "data_type"])
        .agg(
            {
                amount_indicator + "_sum": "sum",
                amount_indicator + "_mean": "mean",
    ...
    data, predicted_time_period = get_churn_and_new_comer_columns(
        data, time_p_ind, amount_indicator, customer_indicator
    )
    new_comer_data, churn_data = get_new_comer_churn_data(
        data, customer_indicator, amount_indicator, time_p_ind
    )
    ...
    return (
        data_pv.query("data_type == 'actual'"),
        data_pv.query("data_type == 'prediction'"),
        top_100_data,
        worst_100_data,
        predicted_time_period,
        new_comer_data,
        churn_data,
    )
...
def adding_plots_to_pane(plot_id, hover_data, size):
    return html.Div(
        [dcc.Graph(id=plot_id, hoverData={"points": [hover_data]})],
...
def create_dashboard(
    ...
    try:
        data = get_raw_data(
            time_indicator, amount_indicator, data_source, data_query_path
        )
        ...
        (
            ...
            top_100_customers,
            worst_100_customers,
        ) = get_filters(results, customer_indicator, amount_indicator)
        app.layout = html.Div()
        if len(data) == 0:
            return app
    except Exception as e:
    ...
    # adding filters
    pane_count = (
        int(len(filters) / num_f_p)
        if int(len(filters) / num_f_p) == len(filters) / num_f_p
        else int(len(filters) / num_f_p) + 1
    )
    components = []
    for i in range(pane_count):
        _filters = (
            filters[i * num_f_p : (i + 1) * num_f_p]
            if i != pane_count - 1
            else filters[i * num_f_p :]
        )
        _pane = adding_filter_to_pane(
            [adding_filter(f[0], f[1], f[2], f[3], f[4]) for f in _filters],
            filter_style,
        )
    ...
    def update_graph_4(hover_data):
        ...
        dff = churn_data[
            churn_data[time_indicator + "_per_" + time_period] == starting_date
        ]
        if len(dff) == 0:
            return {
                "data": [],
                "layout": go.Layout(
                    height=600, title="Churn Customers Of Purchase Time Line"
                ),
            }
        else:
            churn_rate = list(
            ...
            return {
                "data": trace,
                "layout": go.Layout(
                    height=600, title=" Churn Rate (%) per " + time_period
                ),
            }
    ...
    def update_graph_4(hover_data):
        starting_date = hover_data["points"][0]["customdata"]
        dff = new_comer_data[
            new_comer_data[time_indicator + "_per_" + time_period] == starting_date
        ]
        if len(dff) == 0:
            return {
                "data": [],
            ...
            trace = [
                go.Pie(
                    labels=["Newcomer User Ratio", "Engaged User Ratio"],
            ...
            return {
                "data": trace,
                "layout": go.Layout(
                    height=600, title=" Newcomer Rate (%) per " + time_period
                ),
            }
    ...
    def update_graph_4(sum_or_avg, hover_data):
        starting_date = hover_data["points"][0]["customdata"]
        value_column = (
            amount_indicator + "_sum"
            if sum_or_avg == "sum"
            else amount_indicator + "_mean"
        )
        dff = churn_data[
            churn_data[time_indicator + "_per_" + time_period] >= starting_date
        ]
        func = (
            {value_column: "sum"}
            if sum_or_avg == "sum"
            else {amount_indicator + "_mean": "mean"}
        )
        dff = (
            dff.groupby(time_indicator + "_per_" + time_period).agg(func).reset_index()
        )
        if len(dff) == 0:
            return {
                "data": [],
            ...
            trace = [
                go.Scatter(
                    x=dff[time_indicator + "_per_" + time_period],
                    y=dff[value_column],
                    mode="markers+lines",
                    name=value_column,
                )
    ...
    def update_graph_1(_customer, sum_or_avg):
        dff = (
            top_100_data[top_100_data[customer_indicator] == _customer]
            if _customer != "all"
            else top_100_data
        )
        value_column = (
        ...
        title = (
            "Top 100 the most Engaged Customers of Average Values per " + time_period
        )
        if sum_or_avg == "sum":
        ...
        if len(dff) == 0:
            ...
            return {"data": [trace], "layout": go.Layout(height=600, title=title)}
    ...
    def update_graph_2(_customer, sum_or_avg):
        dff = (
            worst_100_data[worst_100_data[customer_indicator] == _customer]
            if _customer != "all"
            else worst_100_data
        )
        value_column = (
        ...
        title = (
            "Top 100 the least Engaged Customers of Average Values per " + time_period
        )
        if sum_or_avg == "sum":
            title = title.replace("Average", "Sum")
        if len(dff) == 0:
    ...
    def update_graph_4(sum_or_avg):
        ...
        _prev_time_period = predicted_time_period - datetime.timedelta(
        ...
        if len(actual) == 0 and len(prediction) == 0:
            ...
            trace = [
                go.Scatter(
                    x=actual[time_indicator + "_per_" + time_period],
                ...
                go.Scatter(
                    x=prediction[time_indicator + "_per_" + time_period],
    ...
    def update_graph_4(sum_or_avg, hover_data):
        ...
        if len(dff) == 0:
            return {
                "data": [],
                "layout": go.Layout(
                    height=600, title="Newcomer Customer Value Prediction"
                ),
            }
        else:
            trace = [
