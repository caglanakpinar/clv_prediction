...
class TrainLSTM:
    def __init__(
        ...
        self.params = (
            hyper_conf("next_purchase")
            if check_for_existing_parameters(self.directory, "next_purchase") is None
            else check_for_existing_parameters(self.directory, "next_purchase")
        )
        self.hyper_params = get_tuning_params(
            hyper_conf("next_purchase_hyper"), self.params
        )
        self.optimized_parameters = {}
        self._p = None
        self.order_count = order_count
        self.time_period = time_period
        self.data, self.features, self.c_min_max, self.params = data_manipulation_np(
            date=date,
        ...
        self.future_date = self.max_date + datetime.timedelta(
            days=convert_time_preiod_to_days(self.time_period)
        )
        self.model_data = {
            "x_train": None,
            "y_train": None,
    ...
    def get_model_data(self, customer):
        ...
        try:
            data = arrange__data_for_model(
                df=_data, f=[self.features], parameters=self.params
            )
    ...
    def data_preparation(self):
        ...
        execute_parallel_run(
            self.customers, self.get_model_data, arguments=None, parallel=16
        )
        try_count = 0
        while try_count < 20:
            try:
                ...
                for c in model_data:
                    if self.model_data["x_train"] is not None:
                        for _split in ["x_train", "y_train", "x_test", "y_test"]:
                            self.model_data[_split] = np.concatenate(
                                [self.model_data[_split], model_data[c][_split]]
                            )
                        self.client_sample_sizes.append(
                            model_data[c]["x_train"].shape[0]
                        )
                    else:
                        self.client_sample_sizes.append(
                            model_data[c]["x_train"].shape[0]
    ...
    def build_parameter_tuning_model(self, hp):
        ...
        lstm = layers.LSTM(
            int(hp.Choice("units", self.hyper_params["units"])),
    ...
    def create_prediction_result_data(self):
        ...
        _import_files = glob.glob(
            join(self.directory, "temp_next_purchase_results", "*.csv")
        )
        li = []
        ...
        self.results[self.time_indicator] = self.results[self.time_indicator].apply(
        ...
        self.results = self.results[
            (self.results[self.time_indicator] > self.max_date)
            & (self.results[self.time_indicator] < self.future_date)
        ]
        print(self.results.head())
        print(
            "number of predicted customers :",
            len(self.results[self.customer_indicator].unique()),
        )
    ...
    def initialize_keras_tuner(self):
        ...
        self.hyper_params, self.optimum_batch_size = batch_size_hp_ranges(
            client_sample_sizes=self.client_sample_sizes,
