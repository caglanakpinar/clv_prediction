...
class TrainLSTMNewComers:
    def __init__(
        ...
        self.params = (
            hyper_conf("newcomers")
            if check_for_existing_parameters(self.directory, "newcomers") is None
            else check_for_existing_parameters(self.directory, "newcomers")
        )
        self.hyper_params = get_tuning_params(
            hyper_conf("newcomers_hyper"), self.params
        )
        ...
        self.prev_model_date = check_model_exists(
            self.directory, "trained_newcomers_model", self.time_period
        )
    ...
    def data_preparation(self):
        self.model_data = arrange__data_for_model(
            df=self.data, f=[self.features], parameters=self.params
        )
    ...
    def prediction_execute(self):
        ...
        while self.max_date < self.future_date:
            ...
            self.model_data = arrange__data_for_model(
                self.data, [self.features], self.params
            )
            self.build_model(prediction=True)
            self.learning_process(save_model=False, history=False, show_epochs=False)
            x = arrange__data_for_model(
                self.data, [self.features], self.params, is_prediction=True
            )
            _pred = pd.DataFrame(
                [
                    {
                        self.time_indicator: self.max_date,
                        "order_count": self.model.predict(x)[0][-1],
                    }
                ]
            )
            self.data, self.results = pd.concat([self.data, _pred]), pd.concat(
                [self.results, _pred]
            )
        ...
        self.results[self.features] = self.results.apply(
            lambda row: self.get_actual_value(
                _min=row["min_" + self.features],
                _max=row["max_" + self.features],
                _value=row[self.features],
            ),
            axis=1,
        )
        self.results[self.amount_indicator] = (
            self.results[self.features] * self.average_amount
        )
        ...
        print(
            "result file : ",
            get_result_data_path(self.directory, self.time_period, self.max_date),
        )
