...
class CLV:
    ...
    def __init__(
        ...
        self.time_period = (
            time_period.lower() if data_source is not None else data_source
        )
        self.amount_indicator = amount_indicator
        self.order_count = order_count
        self.customer_indicator = customer_indicator
        self.data_source = (
            data_source.lower() if data_source is not None else data_source
        )
        self.data_query_path = data_query_path
        self.data_query_path_raw = data_query_path
        self.time_indicator = time_indicator
        self.time_schedule = (
            time_schedule.lower() if time_schedule is not None else time_schedule
        )
    ...
    def query_string_change(self):
        ...
        if self.data_source in ["mysql", "postgresql", "awsredshift", "googlebigquery"]:
            self.data_query_path = (
                self.data_query_path.replace("\r", " ")
                .replace("\n", " ")
                .replace(" ", "+")
            )
    ...
    def get_connector(self):
        ...
        try:
            ...
            write_yaml(
                join(self.path, "docs"), "configs.yaml", config, ignoring_aliases=False
            )
            source = GetData(
    ...
    def checking_for_prediction_process(self):
        ...
        if self.job == "prediction":
            models = [
                check_model_exists(self.export_path, m, self.time_period)
                for m in [
                    "trained_next_purchase_model",
                    "trained_purchase_amount_model",
                    "trained_newcomers_model",
            ...
            if self.check_for_time_period():
                if self.check_for_mandetory_arguments():
                    if self.checking_for_prediction_process():
                        ...
                        print(
                            "None of trained model has been detected!. Please run for Train process!"
                        )
                        print(
                            "if these are trained model with .json format please check any of these models of ",
                    ...
                    print(
                        "check for the required paramters to initialize CLV Prediction:"
                    )
    ...
    def check_for_raw_data(self):
        ...
        if self.clv_predicted is not None:
            ...
            self.raw_data["order_seq_num"] = (
                self.raw_data.sort_values(by=self.sorting_columns)
    ...
    def check_for_result_data(self):
        ...
        if self.clv_predicted is not None:
            self.results = pd.concat(
                [
                    self.clv_predicted["purchase_amount"]["results"],
    ...
    def get_result_data(self):
        ...
        return (
            self.results
            if self.clv_predicted is None
            else pd.concat([self.raw_data, self.results])
        )
    ...
    def show_dashboard(self):
        ...
        process = threading.Thread(
            ...
            kwargs={
                "customer_indicator": self.customer_indicator,
