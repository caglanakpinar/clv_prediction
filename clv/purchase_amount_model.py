...
def get_order_freq(predicted_orders, customer_indicator, time_indicator):
    if len(predicted_orders) != 0:
        predicted_orders["order_seq_num"] = (
            predicted_orders.sort_values(by=[customer_indicator, time_indicator])
...
def get_number_of_orders(predicted_orders, customer_indicator):
    ...
    if len(predicted_orders) != 0:
        number_of_orders = (
            predicted_orders.groupby(customer_indicator)
            .agg({"order_seq_num": "max"})
            .reset_index()
        )
...
class TrainConv1Dimension:
    def __init__(
        ...
        self.params = (
            hyper_conf("purchase_amount")
            if check_for_existing_parameters(self.directory, "purchase_amount") is None
            else check_for_existing_parameters(self.directory, "purchase_amount")
        )
        self.hyper_params = get_tuning_params(
            hyper_conf("purchase_amount_hyper"), self.params
        )
        ...
        self.prev_model_date = check_model_exists(
            self.directory, "trained_purchase_amount_model", self.time_period
        )
        self.model_data = {}
        self.result = None
        self.residuals, self.anomaly = [], []
        self.customers = list(self.data[customer_indicator].unique())
        self.predicted_orders = get_order_freq(
            predicted_orders, customer_indicator, time_indicator
        )
        self.num_of_future_orders = get_number_of_orders(
            self.predicted_orders, self.customer_indicator
        )
        self.results = DataFrame()

    def train_test_split(self):
        self.model_data["train"], self.model_data["test"] = random_data_split(
            self.data, float(self.params["split_ratio"])
        )
    ...
    def data_preparation(self, is_prediction=False):
        ...
        self.model_data = reshape_data(
            self.model_data, self.features, self.y, is_prediction
        )

    def build_parameter_tuning_model(self, hp):
        self.input = Input(
            shape=(
                self.model_data["x_train"].shape[1],
                1,
            )
        )
        # conv 1D layer
        conv = layers.Conv1D(
            ...
            kernel_regularizer=regularizers.l1_l2(
                l1=hp.Choice("l1", self.hyper_params["l1"]),
            ...
            activity_regularizer=regularizers.l2(
                hp.Choice("l2", self.hyper_params["l2"])
            ),
        )(self.input)
        conv = layers.BatchNormalization()(conv)
        # LSTM layer
        conv = layers.MaxPooling1D(
            hp.Choice("max_pooling_unit", self.hyper_params["max_pooling_unit"])
        )(conv)
        conv = layers.Dropout(
            hp.Choice("drop_out_ratio", self.hyper_params["drop_out_ratio"])
        )(conv)
        conv = layers.LSTM(
            hp.Choice("lstm_units", self.hyper_params["lstm_units"]),
            use_bias=False,
            activation=hp.Choice("activation", self.hyper_params["activation"]),
            kernel_regularizer=regularizers.l1_l2(
                l1=hp.Choice("l1", self.hyper_params["l1"]),
        ...
        for i in range(
            ...
            conv = layers.Dense(
                hp.Choice("units", self.hyper_params["units"]),
    ...
    def train_execute(self):
        ...
        print(self.model)
        if (
            self.prev_model_date is None
        ):  # previous accepted date for the model file is not found
    ...
    def prediction_per_customer(self, customer):
        ...
        _prediction = get_prediction(
            _prediction_data, _number, self.model.input.shape[1], self.model
        )
        prediction_data[customer] = {}
        prediction_data[customer]["prediction"] = get_predicted_data_readable_form(
            customer,
            _prediction,
    ...
    def rearrange_prediction_data(self):
        self.model_data["prediction_data"] = pd.merge(
            self.model_data["prediction_data"],
            self.num_of_future_orders[[self.customer_indicator, "order_seq_num"]],
            on=self.customer_indicator,
            how="left",
        )
        self.model_data["prediction_data"] = self.model_data["prediction_data"].query(
            "order_seq_num == order_seq_num"
        )
        self.model_data["prediction_data"] = self.model_data[
            "prediction_data"
        ].sort_values(by=self.customer_indicator)
        self.customers = list(
            self.model_data["prediction_data"][
                self.model_data["prediction_data"].isin(
                    list(self.num_of_future_orders[self.customer_indicator].unique())
                )
            ][self.customer_indicator].unique()
        )

    def get_user_of_historic_data(self, sample_customers):
        _sample_p_data = self.model_data["prediction_data"][
            self.model_data["prediction_data"][self.customer_indicator].isin(
                sample_customers
            )
        ]
        _c_min_max = np.array(
            self.c_min_max.sort_values(by=self.customer_indicator)[
                ["user_min", "user_max"]
            ]
        ).tolist()
    ...
    def parallel_prediction(self):
        ...
        for i in range(iters):
            ...
            _result.to_csv(
                join(
                    self.directory,
                    "temp_purchase_amount_results",
                    str(_sample_customers[0] + "_data.csv"),
                ),
                index=False,
            )
    ...
    def create_prediction_result_data(self):
        ...
        _import_files = glob.glob(
            join(self.directory, "temp_purchase_amount_results", "*.csv")
        )
        li = []
    ...
    def prediction_execute(self):
        ...
        self.model_data["prediction_data"] = self.data[
            self.features + [self.customer_indicator]
        ]
        ...
        if self.num_of_future_orders is not None:
            ...
            self.results = merge_0_result_at_time_period(
                self.data,
                self.max_date,
                self.time_period,
                self.customer_indicator,
        ...
        self.results = merging_predicted_date_to_result_data(
            self.results,
        ...
        self.results = check_for_previous_predicted_clv_results(
            self.results,
            self.directory,
            self.time_period,
            self.time_indicator,
    ...
    def batch_size_and_epoch_tuning(self):
        ...
        epoch_bs_combs = list(
            product(
                sorted(self.hyper_params["batch_sizes"], reverse=True),
        ...
        while not optimum_epoch_process_done:
            ...
            if counter >= parameter_tuning_trials:
                optimum_epoch_process_done = True
                loss_values_df = pd.DataFrame(loss_values).sort_values(
    ...
    def update_tuned_parameter_file(self):
        ...
        try:
            ...
            print(e)
            _params = None
        _params["purchase_amount"] = (
            self.params if _params is not None else {"purchase_amount": self.params}
        )
        write_yaml(
            self.directory, "test_parameters.yaml", _params, ignoring_aliases=True
        )
    ...
    def parameter_tuning(self):
        if check_for_existing_parameters(self.directory, "purchase_amount") is None:
            ...
            self.params = check_for_existing_parameters(
                self.directory, "purchase_amount"
            )
