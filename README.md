...
Each model has unique aggregation in order to prepare data to create the model.

***1. Next Purchase Model Data Preparation***
    
- Time difference of each order per customer is calculated. the time difference is calculated regarding of the *time_preiod*. 
...
- Normalized each time difference value related to Min-Max Normalization Method for each customer of Min-Max Normalization individually.
        
- Regarding of *lahead* parameter of the LSTM model,  data set is shaped per customer. 
    
- Iteratively each customer of the data set is collected individually. the process is parallelized according to CPU count.
    
- When model data has been prepared per customer, it is separated according to split ratio into the train and test data set (train_x, train_y, test_x, test_y). 
...
- *order_count* parameter refers us to the model of the feature count.
    
- Previous orders of purchase amounts of each customer are collected from the raw data.
...
- ***Why do we need order count as a feature at Purchase Amount Model?*** 
    ...
    -   ***!!! Why we need an order count of a decision? !!!***
        ...
        -   For instance if we assign order count as 5, user_1, user_2, user_3, ***user_4*** have 100, 101, 300, ***2*** orders.
            ...
            - user_4: only have 2 orders first 9 orders will be 0 (amount) and this will affect the model process.
                
            It is now crucial to have a minimum number of orders that are assigned purchase amounts as 0 just like 1st and 2nd of user_4 are assigned.
            
            However, it is also a crucial point to get as much previous order count as to make kernel size larger.
            
            ***The order count must be optimized even sending to the platform as an argument. If this argument is not using, the platform handles for deciding oforder_count***.
...
- ***Parameter Tuning***
    ...
    - Parameters of networks (LSTM NN & ! Dimensional Conv NN) are tuned via Keras Turner Library. However, *batch_size* and *epoch* are tunned individually.
    ...
    - *epoch* and *batch_size* are iteratively checking by loss values of last epoch via applying Keras- TensorFlow API history. 
      This iteration will be processed until the iteration is lower than *paramter_tuning_trials*.
...
These are the churn customers of the selected date.# Customer Lifetime Value Prediction
